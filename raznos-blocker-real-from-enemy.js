...
#board .rz-real-art .tile-short,#board .rz-real-art .tile-name,#board .rz-real-art .blocker-short,#board .rz-real-art .blocker-name{display:none!important;opacity:0!important;visibility:hidden!important}
#board .rz-real-art .rz-surface{position:absolute!important;inset:-2px!important;border-radius:inherit!important;overflow:hidden!important;z-index:1!important;pointer-events:none!important;background-position:center!important;background-repeat:no-repeat!important;background-size:100% 100%!important;transform:translateZ(0)!important;backface-visibility:hidden!important;will-change:transform}
#board .rz-real-art .rz-overlay{position:absolute!important;inset:0!important;z-index:2!important;border-radius:inherit!important;pointer-events:none!important;box-shadow:inset 0 0 0 1px rgba(255,255,255,.06)!important}
...
function burst(ms=1400){const end=performance.now()+ms;const step=n=>{schedule();if(n<end)requestAnimationFrame(step)};requestAnimationFrame(step)}
function bind(){const b=document.getElementById('board');if(b&&!b.__rzRealObs){b.__rzRealObs=1;new MutationObserver(()=>schedule()).observe(b,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['class','style']})}}
async function start(ms=1800){await loadArt();bind();burst(ms)}
document.addEventListener('DOMContentLoaded',()=>start(2200));window.addEventListener('load',()=>start(2400));setTimeout(()=>start(2000),0);setTimeout(()=>burst(1600),350);document.addEventListener('click',()=>burst(1400),true);document.addEventListener('touchend',()=>burst(1400),true);new MutationObserver(()=>{bind();schedule()}).observe(document.documentElement,{childList:true,subtree:true});})();