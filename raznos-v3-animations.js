...
async function animateMatch(groups){
  ...
  const bd=board();
  if(bd){bd.classList.remove('rz-board-glow-soft');void bd.offsetWidth;bd.classList.add('rz-board-glow-soft');setTimeout(()=>bd.classList.remove('rz-board-glow-soft'),520)}
...
function init(){addStyle();patchResolve();setTimeout(patchResolve,500);setTimeout(patchResolve,1200)}
